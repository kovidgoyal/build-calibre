...
class Freeze(object):
    ...
    def create_skeleton(self):
        ...
        for x in glob.glob(join(CALIBRE_DIR, 'icons', 'icns', '*.iconset')):
    ...
    def add_calibre_plugins(self):
        ...
        os.mkdir(dest)
        for f in glob.glob(self.ext_dir + '/*.so'):
            shutil.copy2(f, dest)
    ...
    def add_podofo(self):
        print('\nAdding PoDoFo')
        pdf = join(PREFIX, 'lib', 'libpodofo.0.9.4.dylib')
    ...
    def add_imaging_libs(self):
        ...
        for x in ('jpeg.8', 'png16.16', 'webp.6'):
            self.install_dylib(os.path.join(PREFIX, 'lib', 'lib%s.dylib' % x))
        self.install_dylib(os.path.join(PREFIX, 'bin', 'optipng'), False)
    ...
    def add_site_packages(self):
        ...
        sys_path = cPickle.loads(subprocess.check_output([
            PYTHON, '-c', 'import sys, cPickle; cPickle.dump(sys.path, sys.stdout)']))
    ...
    def create_gui_apps(self):
        input_formats = sorted(cPickle.loads(
            subprocess.check_output([
                join(self.contents_dir, 'MacOS', 'calibre-debug'), '-c',
                'from calibre.customize.ui import all_input_formats; import sys, cPickle; sys.stdout.write(cPickle.dumps(set(all_input_formats())))'
            ])
        ))
